/* OpenFOV vendored copy of opentrack/contrib/npclient/npclient.c
 *
 * Original source:
 *   https://github.com/opentrack/opentrack/blob/master/contrib/npclient/npclient.c
 *
 * Originally written by Michal Navratil (uglyDwarf) as part of linuxtrack
 *   https://github.com/uglyDwarf/linuxtrack
 *
 * Copyright (c) 2015 uglyDwarf
 *
 * Licensed under the MIT License — see LICENSE-NPCLIENT.txt in this directory.
 *
 * This file is vendored verbatim apart from this header comment. Any change
 * we need on top is applied as a patch by build.ps1, not by editing this
 * file directly — that keeps drift from upstream visible.
 */
...
static __inline void enhance(unsigned char buf[], unsigned size, unsigned char tbl[], unsigned table_size)
{
    unsigned table_ptr = 0;
    unsigned char var = 0x88;
    unsigned char tmp;

    if (size <= 0 || table_size <= 0 || buf == NULL || tbl == NULL)
        return;

    do
    {
        tmp = buf[--size];
        buf[size] = tmp ^ tbl[table_ptr] ^ var;
        var += size + tmp;
        ++table_ptr;
...
NP_EXPORT(int) NP_GetData(tir_data_t * data)
{
    ...
    data->roll  = (float)clamp_(r);
    data->pitch = (float)clamp_(p);
    data->yaw   = (float)clamp_(y);

    data->tx = (float)clamp_(tx);
    data->ty = (float)clamp_(ty);
    data->tz = (float)clamp_(tz);
...
/* NP_GetSignature (NPCLIENT.10) — XOR-decoded NaturalPoint TrackIR signature */
...
NP_EXPORT(int) NP_RegisterWindowHandle(HWND hwnd) { UNUSED(hwnd); return 0; }
/* NP_RequestData (NPCLIENT.15) */
NP_EXPORT(int) NP_RequestData(unsigned short req) { UNUSED(req); return 0; }
/* NP_SetParameter (NPCLIENT.16) */
NP_EXPORT(int) NP_SetParameter(int a, int b) { UNUSED(a); UNUSED(b); return 0; }
